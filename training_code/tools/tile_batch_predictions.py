...
def predict_tile(tile, weights_path, device, mean, std):
    ...
    return remove_small_components_multiclass(pred_class, min_area=400)

# ------------ per-image pipeline ------------
def process_image(img_path, save_dir, weights_path, device, mean, std, tile_size=1024, overlap=256):
    ...
    for idx, tile in enumerate(tiles):
        pred_tile = predict_tile(tile, weights_path, device, mean, std)
...
def process_folder(img_folder, save_dir, weights_path, device, mean, std, tile_size=1024, overlap=256, num_workers=4):
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = {
            executor.submit(
                ...
                save_dir, weights_path, device, mean, std, tile_size, overlap
