...
for fname in tqdm(all_files):
    ...
    det_stats = compute_detection_metrics_multi_threshold(gt_mask=gt_mask, pred_mask=pred_mask, COLOR_MAP=COLOR_MAP, iou_thresh=0.0)
...
for cname, g in df.groupby("class_name"):
    ...
    for th in thresholds:
        class_dict[f"GT_objects_filt_{th}"] = g.get(f"GT_objects_filt_{th}", pd.Series()).sum()
        class_dict[f"Pred_objects_filt_{th}"] = g.get(f"Pred_objects_filt_{th}", pd.Series()).sum()
        TP, FP, FN = g.get(f"TP_{th}", pd.Series()).sum(), g.get(f"FP_{th}", pd.Series()).sum(), g.get(f"FN_{th}", pd.Series()).sum()
        class_dict.update({
            f"TP_{th}": TP,
            f"FP_{th}": FP,
            f"FN_{th}": FN,
