...
# root_dir = "D:\cracks\Semantic-Segmentation of pavement distress dataset\Combined\ASPHALT_ACCEPTED\SPLITTED\TEST"
# --- Example multiple folders ---
image_dirs = [
    # os.path.join(root_dir, 'process_distress_og'),
    os.path.join(root_dir, 'ACCEPTED_IMAGES')
]

orig_mask_dirs = [
    os.path.join(root_dir, 'ACCEPTED_MASKS'),
    # os.path.join(root_dir, 'RESULTS_AUGMENT')
]

pred_mask_dirs = [
    os.path.join(root_dir, 'process_distress_results'),
    # os.path.join(root_dir, 'ACCEPTED_RESULTS')
]
...
class ImageMaskViewerOptimized:
    ...
    def update_display(self):
        ...
        self.axs[0].imshow(img);          self.axs[0].set_title("Image")
        self.axs[1].imshow(orig_mask);    self.axs[1].set_title("Original Mask")
        self.axs[2].imshow(pred_mask);    self.axs[2].set_title("Predicted Mask")
        self.axs[3].imshow(overlay);      self.axs[3].set_title("Overlay")
        if pcams_img is not None:
            self.axs[4].imshow(pcams_img); self.axs[4].set_title("Pcams (LL)")
        else:
