...
def main(args):
    ...
    results_file = "D_S_segformer/{}-results.txt".format(record_time)
    ...
    train_loss = []
    dice_coefficient = []
    img_save_path = "D_S_segformer/{}-visualization.svg".format(record_time)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.save_best is True:
            torch.save(model.state_dict(), "D_S_segformer/{}-best_model.pth".format(record_time))
            best_model_info = "D_S_segformer/{}-best_model_info.txt".format(record_time)
            with open(best_model_info, "w") as f:
                f.write(train_info + val_info)
        else:
            torch.save(model.state_dict(), "D_S_segformer/model_{}.pth".format(epoch))
